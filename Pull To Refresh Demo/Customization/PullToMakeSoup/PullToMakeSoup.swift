...
class SoupView: UIView, IQAnimatableRefresh {
    ...
    func startLoading() {
        ...
        bubbleTimer = Timer.scheduledTimer(timeInterval: 0.12, target: self, selector: #selector(SoupView.addBubble), userInfo: nil, repeats: true)
    ...
    func releasingAnimation(_ progress: CGFloat) {
        ...
        circle.center = CGPoint(x: frame.size.width / 2, y: refreshHeight / 2 + refreshHeight - (refreshHeight * progress))
